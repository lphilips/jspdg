var Stip = (function () {
    ...
    var handleFuncDeclaration = function (graphs, node, upnode) {
        ...
        /* if the tier of the function can be decided already and the function is an anonymous one
           (thus generated by the pre analysis), make sure that it is called by the same tier.
           This is because the generated call to this function is not analysed by stip, but is
           merely added for the static analysis */
    ...
    var addCallDep = function (from, to) {
        ...
        if (fTypeFrom && fTypeTo) {
            ...
            if (fTypeFrom === DNODES.SERVER && fTypeTo === DNODES.CLIENT)
                to.serverCalls += 1;
            else if (fTypeFrom === DNODES.CLIENT && fTypeTo === DNODES.SERVER) 
                to.clientCalls += 1;
        }
        else
            from.addEdgeOut(to, EDGES.CALL)
