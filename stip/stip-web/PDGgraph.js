function drawLinks(PDG, element, ww,slicededitor) {
  ...
  var dot = "digraph PDG {";//rankdir=LR;\n";
  nodes.forEach(
    function (node, i) {
      var label = node.id,
	  ...
	  if(node.isFormalNode)
			label += " " + node.name
	  if(node.parsenode) 
       		label += " " + ((parsed && parsed.toString().length > 15) ? parsed.toString().slice(0,15)+"..." : parsed) ;//+ " " + node.parsenode.type;
    
    if (node.isEntryNode)
      dot += node.id + "[ label=\"" + label +"\", penwidth=\"2.5\",color=\"#72777A\"];\n";  
    if (node.isDistributedNode)
      dot += node.id + "[ label=\"" + label +"\", style=\"filled\",fillcolor=\"#F56105\"];\n";
    else
