var ARITY = require('../PDG/node.js').ARITY;

var toreturn = {};


function addPlacementTag(fnode, pdg) {
    var fnodes = pdg.getFunctionalityNodes();
    var clientfuncs = fnodes.filter(function (func) {
        return func.tier == DNODES.CLIENT
    });
    var serverfuncs = fnodes.filter(function (func) {
        return func.tier == DNODES.SERVER
    });

    var datadepsC = 0;
    var datadepsS = 0;
    var calldepsS = 0;
    var calldepsC = 0;

    clientfuncs.map(function (func) {
        calldepsC += fnode.countEdgeTypeTo(EDGES.REMOTEC, func.ftype);
        datadepsC += fnode.countEdgeTypeTo(EDGES.REMOTED, func.ftype);
    });

    serverfuncs.map(function (func) {
        calldepsS += fnode.countEdgeTypeTo(EDGES.REMOTEC, func.ftype);
        datadepsS += fnode.countEdgeTypeTo(EDGES.REMOTED, func.ftype, -1);
    });

    if (calldepsC + datadepsC > calldepsS + datadepsS) {
        fnode.tier = DNODES.CLIENT;
    }
    else if (calldepsC + datadepsC < calldepsS + datadepsS) {
        fnode.tier = DNODES.SERVER;
    }
    else {
        fnode.tier = DNODES.CLIENT;
    }
}


toreturn.addPlacementTag = addPlacementTag;


global.DefaultPlacementStrategy = toreturn;
module.exports = toreturn;