...
function transformCallExp(transpiler) {
    ...
    actual_ins.map(function (a_in) {
        a_in.getOutNodes(EDGES.CONTROL)
            .filter(function (n) {
                return n.isCallNode && !n.equals(node)
            })
            .map(function (n) {
                callargs++;
...
function transformForStm(transpiler) {
    var node = transpiler.node,
        body = [],
        parsenode = node.parsenode,
        init = node.getOutNodes(EDGES.CONTROL).filter(function (n) {
            return n.parsenode.equals(parsenode.init);
        }),
        update = node.getOutNodes(EDGES.CONTROL).filter(function (n) {
            return n.parsenode.equals(parsenode.update);
        }),
        test = node.getOutNodes(EDGES.CONTROL).filter(function (n) {
            return n.parsenode.equals(parsenode.test);
        }),
        bodynodes = node.getOutNodes(EDGES.CONTROL)
            .filter(function (n) {
                return n.isEntryNode || n.parsenode.equals(parsenode.body);
            }),
        transpiled;
...
function transformForInStm(transpiler) {
    var node = transpiler.node,
        body = [],
        parsenode = node.parsenode,
        left = node.getOutNodes(EDGES.CONTROL).filter(function (n) {
            return n.parsenode.equals(parsenode.left);
        }),
        right = node.getOutNodes(EDGES.CONTROL).filter(function (n) {
            return n.parsenode.equals(parsenode.right);
        }),
        bodynodes = node.getOutNodes(EDGES.CONTROL)
            .filter(function (n) {
                return n.isEntryNode || n.parsenode.equals(parsenode.body);
...
function transformObjectExp(transpiler) {
    ...
    prop.map(function (property) {
        if (nodesContains(transpiler.nodes, property) && !(Aux.isExpStm(property.parsenode) && Aux.isAssignmentExp(property.parsenode.expression))) {
            transpiled = Transpiler.transpile(Transpiler.copyTranspileObject(transpiler, property));
...
function transformNewExp(transpiler) {
    var node = transpiler.node,
        upnode = node.getInNodes(EDGES.DATA).concat(node.getInNodes(EDGES.CONTROL).filter(function (n) {
            return n.isActualPNode
        }))[0],
        call = node.isCallNode ? node : upnode.getOutNodes(EDGES.CONTROL)
            .filter(function (n) {
                return n.isCallNode && n.parsenode.equals(node.parsenode)
            })[0],
        parsenode = node.parsenode,
...
function transformProperty(transpiler) {
    var node = transpiler.node,
        ...
        exps = node.getOutNodes(EDGES.CONTROL)
            .filter(function (n) {
                return n.isStatementNode && !n.isObjectEntry && !n.isCallNode
            }),
