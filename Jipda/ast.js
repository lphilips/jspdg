...
Ast.isFunctionDeclaration =
  ...
  Ast.nodes = function(n)
  {
    ...
    {
      return [n].concat(Ast.nodes(children(n)));
    }
  }
...
Ast.createAst =
  function (source, config)
  {
    ...
    var ast = esprima.parse(source, {loc: (config ? config.loc : false), owningComments : config.owningComments, comment : true, tokens: true, range : true});
...
//      print(n, "start", n.loc.start.line, n.loc.start.column, "end", n.loc.end.line, n.loc.end.column, "covers", line, col, covers(n));
  ...
  {
    return Arrays.contains(node, children(parent));
  }
...
Ast.scopeInfo =
  function (nodeWithBody)
  {
    ...
    function helper(node)
    {
      ...
      {
        if (Ast.isIdentifier(node.argument))
        {
  ...
  function (scopeInfo)
  {
    ...
    scopeInfo.slice(0).reverse().forEach(
      function (item)
      {
        var name = item.name;
        var exists = false;
        if (Ast.isFunctionDeclaration(item.node))
        {
        ...
        else if (Ast.isVariableDeclarator(item.node))
        {
