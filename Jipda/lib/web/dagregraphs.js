...
function createDagreGraph(nodes, edges)
   {
      var states = nodes.map(
        function (state, id)
        {
          var label = "";
          if (state.completion && state.completion !== "normal")
          {
            label += state.completion + " ";
          }
          if (state.node)
          {
            label += String(state.node).substring(0,20);
          }
    ...
    var transitions = edges.map(function (transition, edgeId)
    {
      var g = transition.g;
      var edge;
      var label = "";
      label += g;
      return {id: edgeId, source: Arrays.indexOf(transition.source, nodes), target: Arrays.indexOf(transition.target, nodes),
        label: label}
    });
   ...
   function drawDagreGraph(graph, states, transitions)
   {
