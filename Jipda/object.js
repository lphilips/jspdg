  ...
  function strongUpdateFrame(frame, name, value)
  {
    ...
    frame.iterateEntries(
      function (entry)
      {
        ...
        {
          newFrame = newFrame.put(entryName, entry.value);
        }
      });
...
//      other.frame.iterateEntries(function (entry) {frame = weakUpdateFrame(frame, entry.key, entry.value)});
//      result.frame = frame;
...
//    }
  ...
  Obj.prototype.addresses = 
    function ()
    {
      var callAddresses = this.Call.values().flatMap(function (Call) {return Call.addresses()});
      var prototypeAddresses = this.Prototype.addresses();
      var frameAddresses = this.frame.values().flatMap(function (value) {return value.addresses()});
      var addresses = callAddresses.concat(prototypeAddresses).concat(frameAddresses).toSet();
      return addresses;
    }
